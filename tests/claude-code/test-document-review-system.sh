...
mkdir -p docs/petropowers/specs

# Create a spec document WITH INTENTIONAL ERRORS for the reviewer to catch
cat > docs/petropowers/specs/test-feature-design.md <<'EOF'
# Test Feature Design
...
Then review the spec at $TEST_PROJECT/docs/petropowers/specs/test-feature-design.md using the criteria from that template.
