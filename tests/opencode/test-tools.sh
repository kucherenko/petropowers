...
if echo "$output" | grep -qi "petropowers:brainstorming\|petropowers:using-petropowers\|Available skills"; then
    echo "  [PASS] find_skills tool discovered petropowers skills"
else
...
# Test 3: Test use_skill with petropowers: prefix
echo ""
echo "Test 3: Testing use_skill with petropowers: prefix..."
echo "  Running opencode with petropowers:brainstorming skill..."

output=$(timeout 60s opencode run --print-logs "Use the use_skill tool to load petropowers:brainstorming and tell me the first few lines of what you received." 2>&1) || {
    exit_code=$?
    if [ $exit_code -eq 124 ]; then
        echo "  [FAIL] OpenCode timed out after 60s"
        exit 1
...
if echo "$output" | grep -qi "brainstorming\|Launching skill\|skill.*loaded"; then
    echo "  [PASS] use_skill loaded petropowers:brainstorming skill"
else
    echo "  [FAIL] use_skill did not load petropowers:brainstorming correctly"
    echo "  Output was:"
    echo "$output" | head -50
    exit 1
fi
